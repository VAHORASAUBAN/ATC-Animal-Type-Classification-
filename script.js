...
async function simpleWorkingAnalysis(imageElement) {
    return new Promise(async (resolve) => {
        ...
        async function analyze() {
            ...
            // Try Local ML classification first, then rule-based fallback
            try {
                console.log('🧠 Attempting local ML analysis...');
                const result = await performLocalMLAnalysis(imageElement);
                ...
                console.warn('❌ Local ML analysis failed:', error);
            }
...
async function performLocalMLAnalysis(imageElement) {
    console.log('🔍 Starting local ML analysis...');
    
    try {
        // Check if local classifier is available
        if (typeof localMLClassifier === 'undefined') {
            throw new Error('Local ML classifier not loaded');
        }

        // Use the local ML classifier
        const result = await localMLClassifier.analyzeImage(imageElement);
        
        if (!result) {
            throw new Error('Local ML analysis returned null');
        }

        console.log('✅ Local ML analysis completed:', result);
        ...
        return {
            ...
            breed: result.breed,
            breedConfidence: Math.round(result.breedConfidence * 100),
        ...
        console.error('❌ Local ML analysis error:', error);
        throw error;
    }
}
